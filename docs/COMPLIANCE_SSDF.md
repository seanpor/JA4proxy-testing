...
| PS.2.1 | Provide integrity verification for releases | Partial | cosign sign-blob signs the binary and SBOM in `deploy/roles/02-artifact-build/tasks/build.yml`; verify happens before systemd start in `deploy/roles/03-ja4proxy-deploy/tasks/main.yml`. **Opt-in**: `ja4proxy_cosign_private_key_path` and `ja4proxy_cosign_public_key_path` default to `""` (unset) so a deploy without configured keys ships an unsigned binary. Ready-to-use when keys are provided; not yet a mandatory step of `make go-live`. See Phase 18-D + Phase 20 P1-6 |
