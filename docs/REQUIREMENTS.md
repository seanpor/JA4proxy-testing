...
| NF-05  | Container images are pinned by SHA-256 digest and a Trivy scan blocks CI on any CRITICAL or HIGH finding.                          | `deploy/roles/09-image-digests`, `deploy/templates/docker-compose.yml.j2`, `.github/workflows/ci.yml`, `scripts/ci/check_digest_regex.py`, `scripts/ci/check_image_scan.py`, `scripts/ci/scan_images.sh` |
