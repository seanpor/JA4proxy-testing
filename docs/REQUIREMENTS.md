...
| F-03  | Enforcement is governed by a single integer dial 0–100 that is hot-reloadable via `SIGHUP` so an operator can escalate without restart. | `deploy/inventory/group_vars/all.yml`, `deploy/templates/ja4proxy.service.j2`, `docs/phases/PHASE_07_VALIDATION_TESTING_complete.md`                  |
