...
| NF-05  | Container images have pinned SHA-256 digests declared in `deploy/expected-image-digests.yml` (weekly-refreshed), and a Trivy scan fails the `image-scan` job on any CRITICAL or HIGH finding. `image-scan` is advisory (not yet a required status check on `main`); a red scan is a merge-blocker signal in PR review. | `deploy/roles/09-image-digests`, `deploy/expected-image-digests.yml`, `deploy/templates/docker-compose.yml.j2`, `.github/workflows/ci.yml`, `scripts/ci/check_digest_regex.py`, `scripts/ci/check_image_scan.py`, `scripts/ci/scan_images.sh` |
